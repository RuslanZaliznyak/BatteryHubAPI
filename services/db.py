...
class Database:
    ...
    def serialize_record(cls, record) -> Dict[str, Any]:
        """
        Serialize a database record to a dictionary.

        Args:
            record: The database record to serialize.

        Returns:
            A dictionary containing the serialized record.
        """
        return {
    ...
    def query_to_db(cls):
        """
          Generate the base query for retrieving records from the database.

          Returns:
              The base query object for database operations.
          """
    ...
    def get_or_create_record(cls, model, field_name, value) -> Any | None:
        """
        Get or create a record in the specified table.

        Args:
            model: The database model representing the table.
            field_name: The name of the field to check for existence.
            value: The value to search or create.
    ...
    def add_record(cls, record: dict) -> dict[str, str] | dict[str, str] | list[dict[str, str | int]]:
        try:
            name_id = cls.get_or_create_record(Name, 'name', record['name'])
            color_id = cls.get_or_create_record(Color, 'color', record['color'])
            voltage_id = cls.get_or_create_record(Voltage, 'voltage', record['voltage'])
            resistance_id = cls.get_or_create_record(Resistance, 'resistance', record['resistance'])
            capacity_id = cls.get_or_create_record(Capacity, 'capacity', record['capacity'])
            weight_id = cls.get_or_create_record(Weight, 'weight', record['weight'])
            source_id = cls.get_or_create_record(Source, 'source', record['source'])
    ...
    @classmethod
    def get_records_by_sorting(cls, res: dict) -> List[Dict[str, Any]]:
        """
        Retrieves records from the database based on sorting conditions.

        Args:
            res (dict): A dictionary containing the sorting conditions.
    ...
    def get_record_by_barcode(cls, barcode: int) -> Dict[str, Any]:
        ...
        record = cls.query_to_db().filter(BatteryData.barcode == barcode).first()
    ...
    def delete(cls, barcode: int) -> Dict[str, Any]:
        ...
        try:
            record = db.session.query(BatteryData).filter(
                BatteryData.barcode == barcode).first()
